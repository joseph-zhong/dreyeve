...
if __name__ == '__main__':
    ...
    # dreyeve_dir = '/home/aba/dreyeve/data/'  # local
    # dreyeve_dir = '/gpfs/work/IscrC_DeepVD/dabati/dreyeve_semantic_seg/data/'  # cineca
    dreyeve_dir = '/home/josephz/ws/var/shared/shared/data/ml/raw/dr-eye-ve/test'

    # local
    data_dir = os.path.join(dreyeve_dir, '{:02d}/frames'.format(int(args.sequence)))
    out_dir = os.path.join(dreyeve_dir, '{:02d}/semseg'.format(int(args.sequence)))
    assert os.path.exists(data_dir), 'Assertion error: path {} does not exist'.format(data_dir)
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)

    # image_list = glob(join(data_dir, '*.jpg'))
    image_list = glob(join(data_dir, '*.png'))
    ...
    for img in tqdm(image_list):
        out = join(out_dir, os.path.basename(img)[0:-4]) + '.npz'
        print(out)
        ...
        np.savez_compressed(out, y_resized)
